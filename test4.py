...
def score_calculate(flag):
    ...
    if flag=='whole':
        ...
        rf = RandomForestClassifier(random_state=1113)
        rf.fit(data, mark)
        algorithm["RF"] = rank_to_dict(rf.feature_importances_, names)
    r = {}
    for name in names:
    ...
    fea_matrix.to_csv('/Users/hhy/Desktop/test/fea_importance_'+flag+'.csv',encoding='utf-8-sig',header=methods)
    return algorithm
algorithm=score_calculate('rf')
label=[]
for num in mark:
    label.append([num])
...
Data.to_csv('/Users/hhy/Desktop/test/Datarf'+str(tmp)+'.csv',encoding='utf-8-sig',header=False,index=False)
